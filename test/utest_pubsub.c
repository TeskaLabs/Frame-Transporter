...
START_TEST(ft_pubsub_1_utest)
{
	...
	ok = ft_subscriber_unsubscribe(&sub1);
	ck_assert_int_eq(ok, true);
...
START_TEST(ft_pubsub_3_utest)
{
	...
	ok = ft_subscriber_unsubscribe(&sub2);
	ck_assert_int_eq(ok, true);
...
START_TEST(ft_pubsub_4_utest)
{
	...
	ok = ft_subscriber_subscribe(&sub1, &context.pubsub, ft_pubsub_utest_TOPIC1);
	ck_assert_int_eq(ok, true);
