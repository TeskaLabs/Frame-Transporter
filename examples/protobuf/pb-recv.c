...
bool on_read(struct ft_stream * established_sock, struct ft_frame * frame)
{
	...
	AMessage * msg = amessage__unpack(NULL, ft_vec_len(vec), ft_vec_ptr(vec));
	if (msg != NULL)
	{
		FT_INFO("Received msg(%zd): a=%d b%c=%d", ft_vec_len(vec), msg->a, msg->has_b, msg->b);
		amessage__free_unpacked(msg, NULL);
	}
	else
	{
		FT_WARN("Error unpacking an incoming message");
	}
